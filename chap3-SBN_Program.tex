...
Along with the global volume of a detector which is defined by it's dimensions, there are usually two other volumes associated with \glspl{lartpc}; the \textit{active} volume and the \textit{fiducial} volume. The active volume is defined as the volume which is enclosed within the \gls{tpc} which is the volume of the detector which allows for event detection. The fiducial volume is not directly defined by the physical design of the detector, but is instead the region within the active volume where event reconstruction is thought to be well understood. Events occurring right at the edge of the \gls{tpc} may be difficult to reconstruct and are therefore poorly understood. Additionally, background events tend to occur close to the edge of the \gls{tpc}, so by only considering events within the fiducial volume, the percentage of well understood signal events is maximised \cite{PDG_fiducial_vol}. The active and fiducial volumes used in each of the \gls{sbn} detectors is defined in \AppendixRef{app:Detector_Volumes}.
...
\subsubsection{Electron Lifetime}
 ...
 \begin{equation}
     Q = Q_{0}e^{-t/\tau_e},
 \end{equation}
where $Q$ is the charge remaining after correcting for electron lifetime, $Q_0$ is the initial charge and $t$ is the drift time \cite{ArgoNeuT_electron_lifetime_paper}. 
...
\end{subfigure}
\begin{subfigure}{0.3\linewidth}
\centering \hspace{-0.6cm} \textbf{CC DIS} \\ \vspace{0.3cm}
\resizebox{\linewidth}{4cm}{
\begin{feynman}
...
\begin{feynman}
    ...
    \weak[color=000000, label=$W^+$, labelDistance=0.3]{24.00, 10.30}{24.00, 12.00}
    \fermion[label=$\pi^+$, labelDistance=0.4]{24.30, 10.00}{26.00, 10.50}
    \fermion[label=$A$, labelDistance=0.2]{24.30, 10.00}{26.00, 9.50}
    \parton{24.00,10.00}{0.30}
\end{feynman}
}
\end{subfigure}
...
\end{subfigure}
\begin{subfigure}{0.3\linewidth}
\centering \hspace{-0.6cm}\textbf{NC DIS} \\ \vspace{0.3cm}
\resizebox{\linewidth}{4cm}{
\begin{feynman}
...
\begin{feynman}
    ...
    \fermion[label=$A$, labelDistance=0.4]{22.00, 9.00}{23.70, 10.00}
    \weak[color=000000, label=$Z^0$, labelDistance=0.3]{24.00, 10.30}{24.00, 12.00}
    \fermion[label=$\pi^0$, labelDistance=0.4]{24.30, 10.00}{26.00, 10.50}
    \fermion[label=$A$, labelDistance=0.2]{24.30, 10.00}{26.00, 9.50}
    \parton{24.00,10.00}{0.30}
\end{feynman}
}
